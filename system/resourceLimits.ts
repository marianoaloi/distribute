...
// Shared by compareImg/mediaIndexer.ts and compareImg/videoFrames.ts -
// their concurrency caps (in-flight media items, concurrent ffmpeg
// extractions) used to be sized only from os.cpus().length, which says
// nothing about whether the machine actually has the RAM to run that many
// at once. On a loaded machine (see the 2026-08-25 investigation: 20 cores
// but ~3GB free) that let the pipeline oversubscribe itself into
// multi-minute stalls that then tripped videoFrames' timeouts - not because
// any file was bad, but because everything was starved for memory at once.
//
...
export const TASK_MEMORY_ESTIMATE = {
    // NOTE: there is deliberately no `pixelHash` entry any more. Pixel
    // fingerprinting used to be the single hungriest task here (~717MB peak
    // for one 21MP image, because Jimp decoded the whole thing to RGBA just
    // to keep a 40x40 crop) and needed a memory-aware worker pool to stay
    // survivable. compareImg/pixelHash.ts now has ffmpeg decode straight to
    // 48x48, so the task costs a few MB in a short-lived child process and
    // no longer needs rationing at all.
    //
